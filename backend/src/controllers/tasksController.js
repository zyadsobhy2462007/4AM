...
async function adminListAllTasks(req, res) {
  try {
    ...
    if (isPostgres) {
      const result = await dbModule.query(`
        SELECT t.id, t.user_id, t.title, t.description, t.status, t.week_start, t.assigned_by, t.created_at, t.completed_at,
...
async function updateTaskStatus(req, res) {
  try {
    ...
    if (isPostgres) {
      ...
      const r = await dbModule.query('SELECT id,user_id,title,description,status,week_start,assigned_by,created_at,completed_at FROM tasks WHERE id = $1', [id]);
      ...
      const row = await dbModule.getAsync('SELECT id,user_id,title,description,status,week_start,assigned_by,created_at,completed_at FROM tasks WHERE id = ?', [id]);
      return res.json({ task: row });
    } else {
